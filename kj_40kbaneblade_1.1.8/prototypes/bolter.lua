if not data.raw["ammo"]["kj_bolt"] then
	...
	-- BolterAmmo
		...
		
		{
			type = "ammo",
			name = "kj_bolt",
			icon = "__kj_40kbaneblade__/graphics/equipment/bolt100mk2.png",
			icon_size = 32,
			ammo_type =
			{
			  category = "kj_bolter",
	...
	for iterator,value in ipairs(data.raw["recipe"]["kj_bolt"].ingredients) do 
		data.raw["recipe"]["kj_bolt"].ingredients[iterator][2] = value[2] * settings.startup["kj_40kbanebladeammo_cost_setting_multiplicator"].value
	end	
end
