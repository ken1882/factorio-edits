...
function RemoteView.restore_inventory(player)
  ...
  for i = 1, util.min(#inventory, #playerdata.navsat_inventory) do
    ...
    local filter = playerdata.navsat_inventory_filters[i] --[[@as string|nil]]
    if not items[filter] then
      -- Ensure this filter item still exists. It might be that the mod it's from it was removed.
      playerdata.navsat_inventory_filters[i] = nil
      filter = nil
    end
