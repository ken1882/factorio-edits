...
Krastorio2.name_advanced_condenser_turbine = "se-kr-advanced-condenser-turbine"
Krastorio2.name_advanced_condenser_turbine_generator = "se-kr-advanced-condenser-turbine-generator"
Krastorio2.name_advanced_condenser_turbine_tank = "se-kr-advanced-condenser-turbine-tank"
Krastorio2.advanced_condenser_turbine_tank_offset = 3
...
function Krastorio2.on_entity_created(event)
  ...
  elseif entity.name == Krastorio2.name_advanced_condenser_turbine then
    local direction_vector = Util.vector_multiply(Util.direction_to_vector(entity.direction), -1)
    local tank_position_offset = Util.vector_multiply(direction_vector, Krastorio2.advanced_condenser_turbine_tank_offset)
    local tank_position = Util.vectors_add(entity.position, tank_position_offset)

    local generator = entity.surface.create_entity{
      name = Krastorio2.name_advanced_condenser_turbine_generator,
      position = entity.position,
    ...
    local tank = entity.surface.create_entity{
      name = Krastorio2.name_advanced_condenser_turbine_tank,
    ...
    if tank then
      ...
      game.print("advanced-condenser-turbine-tank error")
    end
  end
end
...
function Krastorio2.on_removed_entity(event)
  if   event.entity
   and event.entity.valid
   and event.entity.surface
   and event.entity.name == Krastorio2.name_advanced_condenser_turbine
  then
    ...
    for _, generator in pairs(list) do
      if generator and string.starts(generator.name, Krastorio2.name_advanced_condenser_turbine_generator) then
        generator.destroy()
      end
    end

    local tank = entity.surface.find_entity(Krastorio2.name_advanced_condenser_turbine_tank, tank_position)
    if tank then tank.destroy() end
  end
end
Event.addListener(defines.events.on_entity_died, Krastorio2.on_removed_entity)
Event.addListener(defines.events.on_robot_mined_entity, Krastorio2.on_removed_entity)
Event.addListener(defines.events.on_player_mined_entity, Krastorio2.on_removed_entity)
Event.addListener(defines.events.script_raised_destroy, Krastorio2.on_removed_entity)

function Krastorio2.on_player_rotated_entity(event)
  if   event.entity
   and event.entity.valid
   and event.entity.surface
   and event.entity.name == Krastorio2.name_advanced_condenser_turbine
  then
    ...
    if tank then
      ...
      local tank_position_offset = Util.vector_multiply(direction_vector, Krastorio2.advanced_condenser_turbine_tank_offset)
      local tank_position = Util.vecotrs_add(entity.position, tank_position_offset)
...
Event.addListener(defines.events.on_player_rotated_entity, Krastorio2.on_player_rotated_entity)

--- Ensures that all turbines in an area on a surface are valid
---@param surface LuaSurface
---@param area BoundingBox
function Krastorio2.reset_surface(surface, area)
