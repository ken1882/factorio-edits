...
if player.character and player.character.valid then
	...
	if LV > 0 then
		...
		if math.random(100)<=chance then 
			...
			local w='rpg-magic-shock'
			if d>5 then w=weap[math.random(#weap)] end
			if w=='rpg-magic-shock' then ShockNear(entity,player,LV)
				else
				if d<10 then w='distractor-capsule' end
				player.surface.create_entity{name=w, target=entity, target_position=entity.position,  position=player.position, force=player.force, speed=0.5}  --0.4
				if w=="rpg_hadouken" then player.play_sound{path="hadouken"} end
