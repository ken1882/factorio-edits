...
heliBase = {
	...
	OnTick = function(self)
		...
		self:updateAirborneStatus()
		self.curState.OnTick(self)

		if self.valid then
			self:handleColliderDamage()
		end
		self.lastAirborne = self.airborne
	end,
	...
	updateAirborneStatus = function(self)
		self.airborne = self.height > maxCollisionHeight
		if self.airborne == self.lastAirborne then
			return
		end
		local player = self.baseEnt.get_driver()
		local heli   = self.baseEnt
		if player == nil then
			player = heli
		end
		if self.airborne then
			self.baseEnt = heliTakeoff(player, heli, game, defines, settings)
		else
			self.baseEnt = heliLand(player, heli, game, defines, settings)
		end
	end,
}
