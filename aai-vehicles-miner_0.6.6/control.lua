...
function Prototypes.item_prototype(name)
  if not Prototypes.item_prototypes[name] then
    Prototypes.item_prototypes[name] = game.item_prototypes[name]
  end
...
function Prototypes.entity_prototype(name)
  if not Prototypes.entity_prototypes[name] then
    Prototypes.entity_prototypes[name] = game.entity_prototypes[name]
  end
...
local get_fuel = {
    ...
    build = function()
        return util.table_map(game.item_prototypes,
            function(name, item)
                local fuel_item = {}
    ...
    build = function()
      ...
      for _, recipe in pairs(game.recipe_prototypes) do
          ...
          for _, product in pairs(recipe.products) do
            if product.name == "empty-barrel" or product.name == "empty-canister" then
              valid = true break
            end
          end
...
local function consume_fuel_or_equipment(miner)
    if miner.entity.grid and miner.entity.grid.available_in_batteries > 10000 then
        ...
        local batteries = util.table_filter(miner.entity.grid.equipment, function(v) return v.type == "battery-equipment" end)
...
local function update_disallowed_resource_categories()
  global.disallowed_resource_categories = {}
  for interface, functions in pairs(remote.interfaces) do
    if functions["aai_vehicles_miner_disallowed_resource_categories"] then
      ...
      for _, resource_category in pairs(added_categories) do
        global.disallowed_resource_categories[resource_category] = interface
      end
    end
  end
end
