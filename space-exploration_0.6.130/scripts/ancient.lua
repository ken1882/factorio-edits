...
function Ancient.get_next_vault_loot(force)
  local force_loot_bag = global.forces[force.name].vaults_loot_bag
  local items = {"effectivity-module-9", "speed-module-9", "productivity-module-9"} -- Last element is guaranteed to be found on the 1st vault
  ...
  if not force_loot_bag then
    ...
    if not global.vault_loot_rng then global.vault_loot_rng = game.create_random_generator() end
    local next_loot = table.remove(new_loot_bag) -- Remove last element (prod) from the new bag
    Util.shuffle_with_generator(new_loot_bag, global.vault_loot_rng)
    global.forces[force.name].vaults_loot_bag = new_loot_bag
    Log.debug("First Loot: "..next_loot..". Bag: " .. serpent.line(global.forces[force.name].vaults_loot_bag))
    return next_loot
  else
    ...
    Log.debug("Loot: "..next_loot..". Bag: " .. serpent.line(global.forces[force.name].vaults_loot_bag))
    return next_loot
  end
end
...
function Ancient.make_vault_interior(zone, force)
    ...
    if global.glyph_vaults[zone.glyph][zone.index].surface_index and game.get_surface(global.glyph_vaults[zone.glyph][zone.index].surface_index) then
    ...
    if game.get_surface(surface_name) then
      global.glyph_vaults[glyph][zone.index].surface_index = game.get_surface(surface_name).index
      return
    end
