...
function CoreMiner.register_seam_fissure(resource_set)
  ...
  ---Table of all generated core seam data, indexed by registration number
  ---@type IndexMap<CoreSeamInfo>
...
function CoreMiner.on_entity_destroyed(event)
  ...
  if not global.core_seams_by_registration_number then return end
...
function CoreMiner.remove_colliding_entities(resource_set, collision_mask, surface)
  ...
  local protected_forces = protected_system_forces
    ...
    if entity.valid then
      ...
      if entity ~= resource_set.resource and entity ~= resource_set.fissure then
        ...
        elseif not force_name then
          entity.destroy({raise_destroy = true})
        elseif is_system_force(force_name) and not protected_forces[force_name] and entity.type ~= "spider-leg" then
          local entity_name = entity.name
          entity.destroy({raise_destroy = true})
